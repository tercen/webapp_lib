...
class TercenUtils {
  static Table joinTables(
      Table tbl1, Table tbl2, List<String> onl, List<String> onr,
      {List<String> excludeColumns = const []}) {
    List<List> newColumnValues = [];
    int numCols =
        tbl1.columns.length + tbl2.columns.length - excludeColumns.length;
    ...
    for (var colValuePair
        in IterableZip<dynamic>([columnList, newColumnValues])) {
