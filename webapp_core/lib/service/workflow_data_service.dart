...
import 'package:webapp_core/runner/utils/webapp_table.dart';
...
//FIXME Create a workflow task service
class WorkflowDataService {
  ...
  Future<List<sci.Workflow>> fetchWorkflows(
      {required String projectId,
      String folderId = "",
      bool useCache = true,
      bool recursive = false}) async {
    final key = "kumoWorkflowTable_${projectId}_${folderId}";
    if (useCache && CacheObject().hasCachedValue(key)) {
      return CacheObject().getCachedValue(key);
    } else {
      final workflowList = (await ProjectDataService().fetchProjectObjects(
              projectId: projectId,
              folderId: folderId,
              includeFolders: false,
              recursive: false))
          .whereType<sci.Workflow>()
          .where((wkf) => wkf.isHidden == false)
          .where((wkf) => wkf.isDeleted == false)
          .toList();
      ...
      if (useCache) {
        CacheObject().addToCache(key, workflowList);
      }
  ...
  Future<Workflow> fetchWorfklow(
      {required String id, bool useCache = true}) async {
    var key = "fetchWorkflow_${id}";
    if (useCache && CacheObject().hasCachedValue(key)) {
      return CacheObject().getCachedValue(key);
    }

    final wkf = await tercen.ServiceFactory().workflowService.get(id);

    if (useCache) {
      CacheObject().addToCache(key, wkf);
    }
  ...
  bool workflowHasMetas(
      {required Workflow workflow, List<Pair> metas = const []}) {
    return metas.isEmpty ||
        metas.every((meta) => workflow.getMeta(meta.key) == meta.value);
  }
  ...
  bool _isFileSchema({required Schema schema}) {
    return schema.columns.any((col) => col.name.contains("mimetype"));
  }

  //TODO Likely missing UnionRelation here
  ...
  // Future<List<WebappTable>> fetchStepColumns(String workflowId, String stepId,
  ...
  Future<List<WebappTable>> fetchStepResults(
      {required String workflowId,
      required String stepId,
      bool useCache = true}) async {
    final wkf = await fetchWorfklow(id: workflowId);
    var key = "fetchStepResults_${wkf.id}_$stepId";

    if (CacheObject().hasCachedValue(key) && useCache) {
      return CacheObject().getCachedValue(key);
    }

    final dStp = wkf.steps
    ...
    if (dStp == null) {
      Logger().log(
          level: Logger.WARN,
          message:
              "fetchStepResults :: step id $stepId not found in workflow $workflowId (${wkf.name}). Returning empty table");
      return <WebappTable>[];
    }

    final relList = getSimpleRelations(dStp.computedRelation);

    final schList = await tercen.ServiceFactory()
        .tableSchemaService
        .list(relList.map((e) => e.id).toList());

    final outList = <WebappTable>[];
    final colNameList = <List<String>>[];
    final selectList = <Future>[];
    for (var sch in schList) {
      final colNames = sch.columns.map((e) => e.name).toList();
      colNameList.add(colNames);
      selectList.add(tercen.ServiceFactory()
          .tableSchemaService
          .select(sch.id, colNames, 0, sch.nRows));
    }

    final selectResult = await Future.wait(selectList);
    for (var i = 0; i < selectResult.length; i++) {
      final colNames = colNameList[i];
      final tbl = selectResult[i] as sci.Table;
      final outTbl = WebappTable();
      for (var colName in colNames) {
    ...
    if (useCache) {
      CacheObject().addToCache(key, outList);
    }
  ...
  bool hasRunStep({required sci.Workflow workflow, required String stepId}) {
    final taskState =
        workflow.steps.firstWhere((step) => step.id == stepId).state.taskState;
