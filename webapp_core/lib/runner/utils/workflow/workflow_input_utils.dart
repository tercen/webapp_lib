...
class WorkflowInputUtils {
  sci.Workflow documentIdToTableStep(
      {required sci.Workflow workflow, required String stepId, required String documentId, String stepName = ""}) {
    ...
    sci.Workflow relationToTableStep(
      {required sci.Workflow workflow, required String stepId, required sci.Relation relation, String stepName = ""}) {
    final tableStep = workflow.steps
        .whereType<sci.TableStep>()
        .firstWhere((step) => step.id == stepId, orElse: () => sci.TableStep());
    ...
    tableStep.model.relation =  relation;
    tableStep.state.taskState = sci.DoneState();
  ...
  sci.Workflow relationIdToTableStep(
      {required sci.Workflow workflow, required String stepId, required String relationId, String stepName = ""}) {
    final tableStep = workflow.steps
        .whereType<sci.TableStep>()
        .firstWhere((step) => step.id == stepId, orElse: () => sci.TableStep());
    ...
    tableStep.model.relation =  sci.SimpleRelation()..id;
    tableStep.state.taskState = sci.DoneState();
