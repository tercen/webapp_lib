...
class WorkflowListComponent extends ActionListComponent {
  ...
  Widget createWidget(BuildContext context, WebappTable table) {
    ...
    for (var ri in getSortedRowIdx(table) ) {
      ...
      includeEntry = includeEntry && (showDone || (row[statusColName][0] != "Done" && row[statusColName][0] != "Unknown"));
      if (includeEntry  ) {
  ...
  Widget createEntry(List<dynamic> values, List<ListAction> actions, int rowIdx, WebappTable rowValues){
    ...
    var textColor = rowValues[detailColumn].first == "" ? Styles()["black"] : Styles()["red"];
    ...
    // rowValues
    row = errorWrap(rowIdx, row, rowValues[detailColumn].first, rowValues[detailColumn].first);
